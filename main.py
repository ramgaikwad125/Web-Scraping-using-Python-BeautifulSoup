...
try:
    nifty50_close_price = soup.find_all(
        "span", class_="tile-current-price pr05r LpriceCP LpriceNP positive")[0].string[17:25]
except:
    nifty50_close_price = soup.find_all(
        "span", class_="tile-current-price pr05r LpriceCP LpriceNP negative")[0].string[17:25]
...
try:
    banknifty_close_price = soup.find_all(
        "span", class_="tile-current-price pr05r LpriceCP LpriceNP positive")[0].string[17:25]
except:
    banknifty_close_price = soup.find_all(
        "span", class_="tile-current-price pr05r LpriceCP LpriceNP negative")[0].string[17:25]
...
#     print('In get data')
    ...
    appended_DATA = appended_DATA.rename(columns={'date': 'quote_date',
                                                  'ticker': 'symbol',
                                                  'expiry_date': 'expiration',
                                                  'str_price': 'strike',
                                                  'bid_price_put': 'bid_1545',
                                                  'ask_price_put': 'ask_1545',
                                                  'net_chng_put': 'net_change',
                                                  'iv_put': 'implied_volatility_1545',
                                                  'vol_put': 'volume'
                                                  }, index=str)

    appended_DATA = appended_DATA.replace(
        to_replace=r'^-$', value=0, regex=True).reset_index()
    ...
    appended_DATA['quote_date'] = pd.to_datetime(
        appended_DATA.quote_date, format='%Y-%m-%d')
    appended_DATA['expiration'] = pd.to_datetime(
        appended_DATA.expiration, format='%d%b%Y')
    ...
    appended_DATA['bid_1545'] = pd.to_numeric(
        appended_DATA['bid_1545'], errors='coerce')
    appended_DATA['ask_1545'] = pd.to_numeric(
        appended_DATA['ask_1545'], errors='coerce')
    appended_DATA['volume'] = pd.to_numeric(
        appended_DATA['volume'], errors='coerce')
